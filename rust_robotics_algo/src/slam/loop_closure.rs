...
impl LoopClosureDetector {
    ...
    pub fn detect(
        ...
        let mut closures = self.detect_proximity_based(
            poses,
    ...
    fn detect_proximity_based(
        ...
        if current_pose_idx >= poses.len()
            || current_pose_idx < self.config.min_temporal_separation
        {
            return Vec::new();
        }
    ...
    fn estimate_transform(
        ...
        let (rotation, translation) = self.compute_weighted_rigid_transform(
            &points_current,
            &points_candidate,
            &weights,
        )?;
        ...
        let chi2 = self.compute_alignment_mahalanobis(
    ...
    fn observation_cartesian_covariance(
        ...
        covariance.iter().all(|v| v.is_finite()).then_some(covariance)
    }
    ...
    fn compute_weighted_rigid_transform(
        ...
        for ((source_point, target_point), weight) in
            source.iter().zip(target.iter()).zip(weights.iter().copied())
        {
    ...
    /// Unweighted wrapper retained for the focused rigid-transform unit tests.
    fn compute_rigid_transform(
        ...
        self.compute_weighted_rigid_transform(source, target, &weights)
    }
...
mod tests {
    ...
    fn test_inconsistent_landmark_geometry_is_rejected() {
        ...
        for (landmark_idx, point) in candidate_points.iter().enumerate() {
            graph.add_observation(0, landmark_idx, point.norm(), point.y.atan2(point.x), &covariance);
        }
        for (landmark_idx, point) in current_points.iter().enumerate() {
            graph.add_observation(4, landmark_idx, point.norm(), point.y.atan2(point.x), &covariance);
        }
        let mut config = LoopClosureConfig::default();
        config.proximity_threshold = 0.1;
        config.landmark_observation_gap = 3;
    ...
    fn rejected_observations_do_not_participate_in_loop_closure() {
        ...
        assert!(closures.is_empty(), "flagged observations must not satisfy the common-landmark minimum");
    }
    ...
    fn covariance_weighted_fit_resists_noisy_landmark() {
        ...
        for (landmark_idx, landmark) in landmarks.iter().enumerate() {
            if landmark_idx == 3 {
                let corrupted_point = Vector2::new(15.0, 12.0);
