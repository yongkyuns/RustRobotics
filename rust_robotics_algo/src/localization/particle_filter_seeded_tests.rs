...
fn observation_entry_points_consume_the_same_seeded_stream() {
    ...
    let (z_b, u_b) =
        observation_from_state_with_rng(&truth, &mut dr_b, u, &landmarks, (0.1, MAX_RANGE), &mut b);
    assert_eq!(z_a.len(), 1);
    assert_eq!(z_a, z_b);
    assert_eq!(u_a, u_b);
    assert_eq!(dr_a, dr_b);
    assert_eq!(
        a.next_u64(),
        b.next_u64(),
        "observation draw counts must agree"
    );
}
...
fn rollout(seed: u64) -> Vec<u32> {
    ...
    while time < 50.0 {
        ...
        let covariance = pf_localization_with_rng(
            &mut estimated,
            &mut px,
            &mut pw,
            z,
            (u, 0.1),
            (&mut PFState::default(), &PFNoiseParams::default()),
            &mut rng,
        );
...
fn complete_rollout_replays_every_particle_weight_and_covariance() {
    ...
    assert_eq!(
        first, replay,
        "same seed must replay the entire filter rollout"
    );
...
fn recovery_uses_the_callers_rng_for_every_reset_draw() {
    ...
    let covariance = pf_localization_with_rng(
        ...
        z.clone(),
        (calc_input(), 0.1),
        (&mut state, &PFNoiseParams::default()),
        &mut rng,
    );
    ...
    for _ in 0..2 * NP {
        let _ = rand_with_rng(&mut reference_rng);
    }
    let mut expected = PX::zeros();
    let mut expected_weights = PW::zeros();
    reset_particles_with_rng(
        &mut expected,
    ...
    assert_eq!(
        ...
        "all recovery draws must be accounted for"
    );
