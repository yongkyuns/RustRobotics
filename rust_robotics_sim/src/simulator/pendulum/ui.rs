    ...
    pub fn options(&mut self, ui: &mut Ui, available_policy: Option<&PolicySnapshot>) {
        match self {
            ...
            Self::MPC(model) => {
                ...
                ui.label("Horizon: 12, bounds: ±50N");
            ...
            Self::Policy(policy) => {
                ...
                ui.label("PPO Policy");
                ui.push_id("policy_params", |ui| {
                    Grid::new("policy_param_grid")
                        .num_columns(2)
                        .spacing([10.0, 6.0])
                        .show(ui, |ui| {
                            ui.label("Action std");
                            ui.label(format!("{:.3}", policy.action_std()));
                            ui.end_row();
                        });
                });
...
impl InvertedPendulum {
    ...
    fn show_controller_panel(
        ...
        ui.push_id(self.id, |ui| {
            ComboBox::from_label("")
                .width((controller_width - 32.0).max(140.0))
        ...
        if self.controller_selection == ControllerKind::Policy {
            ...
            ui.collapsing("PPO Trainer", |ui| {
                ...
                if let Some(metrics) = self.trainer_backend.metrics() {
                    ...
                    ui.label(format!(
                        "Upd {}  Step {}  Ep {}",
                        metrics.total_updates, metrics.total_env_steps, metrics.total_episodes
                    ));
                    ui.label(format!(
                        "Ret {:.2}/{:.2}/{:.2}",
    ...
    pub fn options_with_policy(&mut self, ui: &mut Ui) -> bool {
        ...
        ui.group(|ui| {
            ...
            ui.vertical(|ui| {
                ...
                let wide_layout = ui.available_width() > 540.0;

                if wide_layout {
                    ui.horizontal_top(|ui| {
                        let section_gap = 12.0;
                        let total_width = ui.available_width();
                        let left_width = ((total_width - section_gap) * 0.38).clamp(180.0, 240.0);
                        ...
                        ui.allocate_ui_with_layout(
                            ...
                            egui::Layout::top_down(egui::Align::Min),
                            |ui| {
                                let controller_width = ui.available_width().clamp(220.0, 360.0);
                                self.show_controller_panel(
                                    ui,
                                    available_policy.as_ref(),
                                    controller_width,
                                );
                            },
                        );
                    ...
                    self.show_cart_parameters(ui);
                    ui.separator();
                    let controller_width = ui.available_width().clamp(180.0, 280.0);
                    self.show_controller_panel(ui, available_policy.as_ref(), controller_width);
                }
            });
        });
        keep
