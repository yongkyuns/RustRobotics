...
fn frozen_policy_live_and_training_paths_match_noise_resets_and_custom_physics() {
    for (index, plant) in [
        ...
        CartPoleParameters { length_m: 0.6, cart_mass_kg: 0.4, pole_mass_kg: 0.2 },
        CartPoleParameters { length_m: 3.1, cart_mass_kg: 1.7, pole_mass_kg: 2.3 },
    ].into_iter().enumerate() {
        let config = PpoTrainerConfig {
            plant,
            env: PendulumEnvConfig { max_steps: 9, ..Default::default() },
            ..Default::default()
        };
        ...
        let mut sim = InvertedPendulum {
            state, model: plant.model(), trainer_config: config.clone(), ..Default::default()
        };
        ...
        for _ in 0..300 {
            ...
            sim.step_policy_with_rng(PENDULUM_FIXED_DT, &mut b);
            steps += 1;
            if result.done {
                timeouts += usize::from(result.truncated);
                terminals += usize::from(result.terminated());
                observation = env.reset_with_rng(&mut a); steps = 0;
            } else { observation = result.observation; }
        ...
        assert!(terminals > 0 && timeouts > 0, "both episode boundaries must be exercised");
    }
}
...
fn changing_physics_stops_training_and_blocks_stale_policy_until_restart() {
    ...
    assert_eq!(state, sim.state, "a stale snapshot must not drive a changed plant");
    sim.start_training();
...
fn noise_controls_apply_to_both_paths_and_invalidate_existing_training() {
    let mut sim = InvertedPendulum::default(); sim.start_training();
    sim.configure_training_noise(NoiseConfig { enabled: false, scale: 1.0 });
    ...
    sim.configure_training_noise(NoiseConfig { enabled: true, scale: 2.0 });
    assert!(sim.policy_environment_stale);
    assert_eq!(sim.trainer_config.env.action_noise_force_n,
...
fn stopped_policy_still_uses_the_training_time_limit_and_reset_distribution() {
    ...
    sim.start_training(); sim.stop_training();
...
fn old_json_config_defaults_physics_and_custom_physics_roundtrips() {
    ...
    let custom = PpoTrainerConfig {
        plant: CartPoleParameters { length_m: 0.9, cart_mass_kg: 1.6, pole_mass_kg: 0.4 },
        ..Default::default()
    };
    let decoded: PpoTrainerConfig = serde_json::from_str(&serde_json::to_string(&custom).unwrap()).unwrap();
