...
fn every_pooled_stream_uses_the_requested_physical_parameters() {
    let plant = CartPoleParameters { length_m: 0.8, cart_mass_kg: 1.9, pole_mass_kg: 0.3 };
    let cfg = PpoTrainerConfig { plant, ..Default::default() };
    let mut session = PpoTrainerSession::new_seeded_with_environments(cfg, 203, 4);
    assert_eq!(session.env.model(), plant.model());
    for env in &session.additional_environments { assert_eq!(env.env.model(), plant.model()); }
...
fn training_transition_is_shared_nonlinear_rk4_not_linear_euler() {
    let plant = CartPoleParameters { length_m: 1.3, cart_mass_kg: 0.7, pole_mass_kg: 0.4 };
    let cfg = PendulumEnvConfig { action_noise_force_n: 0.0, disturbance_force_n: 0.0,
        disturbance_probability_per_step: 0.0, ..Default::default() };
...
fn nondefault_physics_changes_trajectories_without_changing_initial_networks() {
    ...
    let other = PpoTrainerConfig { plant: CartPoleParameters {
        length_m: 0.5, ..Default::default() }, ..default.clone() };
    ...
    assert_ne!(a.collect_rollout().observations,b.collect_rollout().observations);
}
