...
use crate::{inverted_pendulum::{Model, g}, Vector4};
...
impl Default for CartPoleParameters {
    fn default() -> Self { Self::from(Model::default()) }
}

impl From<Model> for CartPoleParameters {
    fn from(model: Model) -> Self {
        Self { length_m: model.l_bar, cart_mass_kg: model.m_cart, pole_mass_kg: model.m_ball }
    }
}
...
impl CartPoleParameters {
    ...
    pub fn validate(self) {
        assert!([self.length_m, self.cart_mass_kg, self.pole_mass_kg]
            .into_iter().all(|v| v.is_finite() && v > 0.0),
    ...
    pub fn model(self) -> Model {
        self.validate();
        Model { l_bar: self.length_m, m_cart: self.cart_mass_kg,
            m_ball: self.pole_mass_kg, ..Model::default() }
    }
    ...
    pub fn derivative(self, state: Vector4, force_n: f32) -> Vector4 {
        ...
        let acceleration = (force_n + self.pole_mass_kg * sin
            * (g * cos - self.length_m * omega * omega)) / denominator;
        Vector4::new(state[1], acceleration, omega,
    ...
    pub fn step(self, state: Vector4, force_n: f32, dt: f32) -> Vector4 {
        self.validate();
        assert!(dt.is_finite() && dt > 0.0, "cart-pole dt must be finite and positive");
...
mod tests {
    ...
    fn small_signal_matches_classical_linearization() {
        let p = CartPoleParameters { length_m: 1.3, cart_mass_kg: 0.8, pole_mass_kg: 0.4 };
        let (a, b) = p.model().model(1.0);
        for i in 0..4 {
            let mut x = Vector4::zeros(); x[i] = 1e-4;
            let numeric = (p.derivative(x, 0.0) - p.derivative(-x, 0.0)) / 2e-4;
            let mut expected = a.column(i).into_owned(); expected[i] -= 1.0;
    ...
    fn nonlinear_dynamics_satisfy_lagrange_equations_and_power_balance() {
        let p = CartPoleParameters { length_m: 1.7, cart_mass_kg: 0.7, pole_mass_kg: 0.3 };
        ...
        for angle in [-1.2_f32, -0.4, 0.0, 0.7, 2.5] {
            ...
            let cart_residual = (big_m+m)*d[1]-m*l*c*d[3]+m*l*s*x[3]*x[3]-force;
            ...
            let de = (big_m+m)*x[1]*d[1]-m*l*c*(d[1]*x[3]+x[1]*d[3])
                +m*l*s*x[1]*x[3]*x[3]+m*l*l*x[3]*d[3]-m*g*l*s*x[3];
    ...
    fn rk4_refines_toward_independent_small_steps() {
        ...
        let integrate = |steps: usize| {
            let mut x = start;
            for _ in 0..steps { x = p.step(x,1.3,0.2/steps as f32); }
            x
        };
    ...
    fn invalid_physical_configuration_is_rejected() {
        CartPoleParameters { length_m: 0.0, ..Default::default() }.model();
    }
}
