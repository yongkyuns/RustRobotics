...
fn same_rollout(a: &RolloutBatch, b: &RolloutBatch) {
    assert_eq!(
        ...
        "seeded rollout observations"
    );
    assert_eq!(a.latent_actions, b.latent_actions, "seeded rollout actions");
    assert_eq!(
        a.old_log_probs, b.old_log_probs,
        "seeded rollout likelihoods"
    );
...
fn equal_seeds_replay_initial_state() {
    for seed in [0, SEED, u64::MAX] {
        ...
        let _unrelated =
            PolicyNetwork::<AutodiffBackend>::new(&Default::default(), 4, 8, 20.0).snapshot(2.0);
...
fn different_seeds_change_initial_models_and_environment() {
    ...
    assert_ne!(
        a.shared_state().value,
        b.shared_state().value,
        "master seed must affect critic"
    );
    assert_ne!(
        a.env.state(),
        b.env.state(),
        "master seed must affect environment"
    );
...
fn initialization_matches_explicit_uniform_reference() {
        ...
        .zip([(3, 7), (7, 7), (7, 2)])
    {
        ...
        let weight: Vec<f32> = (0..dimensions.0 * dimensions.1)
            .map(|_| reference_rng.gen_range(-bound..bound))
            .collect();
        ...
        assert!(layer
            ...
            .iter()
            .chain(&layer.bias)
            .all(|x| x.is_finite() && x.abs() <= bound));
    }
    assert_eq!(
        cursor(&actual_rng),
        cursor(&reference_rng),
        "initialization draw count"
    );
}
...
fn reset_reference(c: PendulumEnvConfig, rng: &mut StdRng) -> [f32; 4] {
    [
        c.reset_position_range_m,
        c.reset_velocity_range_mps,
        c.reset_angle_range_rad,
        c.reset_angular_velocity_range_radps,
    ]
    .map(|m| draw(rng, m))
}
...
fn observe_reference(x: [f32; 4], c: PendulumEnvConfig, rng: &mut StdRng) -> [f32; 4] {
    let magnitudes = [
        c.observation_position_noise_m,
        c.observation_velocity_noise_mps,
        c.observation_angle_noise_rad,
        c.observation_angular_velocity_noise_radps,
    ];
...
fn environment_draws_match_independent_reference() {
    ...
    for index in 0..64 {
        assert_eq!(
            env.observation_with_rng(&mut rng),
            observe_reference(x, c, &mut reference),
        ...
        assert_eq!(
            ...
            "step observation RNG reference"
        );
        assert_eq!(
            cursor(&rng),
            cursor(&reference),
            "step consumes only caller RNG"
        );
        if step.done {
            x = reset_reference(c, &mut reference);
            assert_eq!(
                env.reset_with_rng(&mut rng),
                observe_reference(x, c, &mut reference),
            ...
            assert_eq!(
                cursor(&rng),
                cursor(&reference),
                "reset consumes only caller RNG"
            );
        }
    }
    assert!(
        disturbances > 0 && disturbances < 64,
        "fixture must exercise both disturbance branches"
    );
}
...
fn resets_advance_rather_than_repeat_the_initial_seed() {
    ...
    for _ in 0..8 {
        assert_eq!(
            a.reset_with_rng(&mut a_rng),
            b.reset_with_rng(&mut b_rng),
            "reset replay"
        );
...
fn noisy_rollouts_replay_every_stored_value() {
    for seed in [0, SEED, u64::MAX] {
        ...
        assert!(
            a.metrics.total_episodes >= 12,
            "fixture must cross reset boundaries"
        );
    }
}
...
fn grouped_and_split_training_calls_replay_real_updates() {
    for seed in [0, SEED, 91] {
        ...
        assert_ne!(
            a.snapshot(),
            initial.policy,
            "fixture must actually train actor"
        );
        assert_ne!(
            a.shared_state().value,
            initial.value,
            "fixture must actually train critic"
        );
...
fn interleaved_unrelated_sessions_do_not_change_replay() {
    ...
    for seed in [301, 302, 303] {
        ...
        other_env.step(3.0);
        let _ =
            PolicyNetwork::<AutodiffBackend>::new(&Default::default(), 4, 3, 20.0).snapshot(2.0);
        candidate.train_updates(1);
    }
    assert_eq!(
        state(&control),
        state(&candidate),
        "independent interleaved sessions"
    );
}
...
fn weight_transfer_preserves_local_random_cursors() {
    ...
    let after = state(&a);
    assert_eq!(
        after.environment_rng, before.environment_rng,
        "weight transfer must retain environment RNG"
    );
    assert_eq!(
        after.action_rng, before.action_rng,
        "weight transfer must retain action RNG"
    );
    assert_eq!(
        after.update_rng, before.update_rng,
        "weight transfer must retain optimizer RNG"
    );
    ...
    assert_eq!(
        state(&a),
        state(&b),
        "replay continues after weight transfer"
    );
}
...
fn optimizer_draw_counts_do_not_change_collection_streams() {
    ...
    assert_eq!(
        cursor(&a.action_rng),
        cursor(&b.action_rng),
        "optimizer must not consume action RNG"
    );
    assert_eq!(
        cursor(&a.environment_rng),
        cursor(&b.environment_rng),
        "optimizer must not consume environment RNG"
    );
...
fn model_size_does_not_consume_live_random_streams() {
    ...
    assert_eq!(
        a.current_observation, b.current_observation,
        "model size must not change environment"
    );
    assert_eq!(cursor(&a.environment_rng), cursor(&b.environment_rng));
    assert_eq!(
        cursor(&a.action_rng),
        cursor(&b.action_rng),
        "model size must not change action stream"
    );
...
fn random_cursors_advance_only_in_their_own_phase() {
    ...
    assert_ne!(
        collected.environment_rng, before.environment_rng,
        "environment cursor must advance"
    );
    assert_ne!(
        collected.action_rng, before.action_rng,
        "action cursor must advance"
    );
    assert_eq!(
        collected.update_rng, before.update_rng,
        "collection must not consume optimizer RNG"
    );
    a.optimize(&batch);
    let optimized = state(&a);
    assert_ne!(
        optimized.update_rng, collected.update_rng,
        "optimizer cursor must advance"
    );
