    ...
    for mean in [-2.0, 0.0, 2.0] {
        for _ in 0..256 {
            ...
            assert_eq!(
                x.action,
                3.0 * x.latent.tanh(),
                "action must be the latent transform"
            );
...
fn tensor_density_and_score_match_independent_reference() {
    ...
    for i in 0..means.len() {
        close(
            data[i],
            reference_log_prob(means[i].into(), latents[i].into(), 2.0, 3.0),
            2e-5,
        );
        close(
            scores[i],
            (f64::from(latents[i]) - f64::from(means[i])) / (2.0_f64 / 3.0).powi(2),
            2e-5,
        );
    }
}
...
fn bounded_entropy_gradient_matches_finite_differences() {
    ...
    for i in 0..ms.len() {
        ...
        close(
            actual[i],
            (reference(&plus) - reference(&minus)) / (2.0 * h),
            5e-5,
        );
    }
}
...
fn clipping_values_and_gradients_cover_both_advantage_signs() {
    ...
    for epsilon in [0.1_f32, 0.2, 0.4] {
        ...
        let reference = |input: &[f64]| -> f64 {
            -input
                ...
                .map(|(log, a)| {
                    let r = log.exp();
                    (r * f64::from(a)).min(
                        r.clamp(1.0 - f64::from(epsilon), 1.0 + f64::from(epsilon)) * f64::from(a),
                    )
                })
                .sum::<f64>()
                / 6.0
        ...
        for i in 0..6 {
            ...
            close(
                actual[i],
                (reference(&plus) - reference(&minus)) / 2e-5,
                2e-5,
            );
        }
    }
...
fn value_coefficient_scales_loss_and_gradient() {
    ...
    for coefficient in [0.0_f32, 0.25, 1.0, 2.0] {
        ...
        for i in 0..3 {
            close(
                actual[i],
                2.0 * f64::from(coefficient) * (f64::from(predictions[i]) - f64::from(targets[i]))
                    / 3.0,
                2e-6,
            );
        }
...
fn session(mean: f32, entropy: f32) -> PpoTrainerSession {
    ...
    session.load_shared_state(&PpoSharedState {
        policy,
        value: ValueSnapshot {
            input,
            hidden,
...
fn rollout(mean: f32, advantage: f32) -> RolloutBatch {
    ...
    RolloutBatch {
        ...
        old_log_probs: latents
            .iter()
            .map(|z| distribution.log_prob(mean, *z))
            .collect(),
...
fn entropy_coefficient_changes_actual_actor_without_advantages() {
    ...
    assert_eq!(
        disabled.snapshot(),
        before,
        "zero advantages and entropy leave fresh actor unchanged"
    );
    assert_ne!(
        enabled.snapshot(),
        before,
        "entropy coefficient must affect actor parameters"
    );
    assert!(
        enabled.snapshot().act([0.0; 4]) < before.act([0.0; 4]),
        "bounded entropy must move a saturated mean toward the center"
    );
...
fn zero_value_coefficient_freezes_existing_adam_momentum() {
    ...
    assert_ne!(
        trained, before,
        "positive value coefficient must train critic"
    );
    session.config.ppo.value_loss_coef = 0.0;
    for _ in 0..3 {
        session.optimize_with_rng(&batch, &mut rng);
        assert_eq!(
            session.shared_state().value,
            trained,
            "zero coefficient must also freeze Adam momentum"
        );
    }
}
...
fn policy_optimizer_follows_latent_action_score() {
    ...
    session.optimize_with_rng(
        &rollout(before, 1.0),
        &mut StdRng::seed_from_u64(0x5050_0005),
    );
    assert!(
        session.policy_latent_mean([0.0; 4]) > before,
        "positive advantage should increase sampled latent likelihood"
    );
}
...
fn deterministic_snapshot_inference_is_unchanged() {
    for mean in [-10.0_f32, -2.0, 0.0, 0.4, 2.0, 10.0] {
        ...
        for obs in [[0.0; 4], [0.1, -0.2, 0.3, 0.4]] {
            ...
            close(
                tensor_scalar(&session.actor.valid().forward(tensor)),
                expected.into(),
                2e-6,
            );
