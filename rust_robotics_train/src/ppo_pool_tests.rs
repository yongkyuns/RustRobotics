...
fn extra_environment_count_does_not_perturb_existing_streams_before_update() {
    ...
    for _ in 0..3 {
        ...
        assert_eq!(
            a.additional_environments[0].env.state(),
            b.additional_environments[0].env.state()
        );
    }
}
...
fn stream_fingerprint(stream: &RolloutEnvironment) -> ([f32; 4], [f32; 4], f32, u64, u64) {
    ...
    (
        stream.current_observation,
        [x[0], x[1], x[2], x[3]],
        stream.episode_return,
        stream.environment_rng.clone().gen(),
        stream.action_rng.clone().gen(),
    )
}
...
fn invalid_pool_sizes_fail_before_any_rollout_allocation() {
    assert!(
        std::panic::catch_unwind(|| PpoTrainerSession::new_seeded_with_environments(
            config(),
            201,
            0
        ))
...
fn constant_session(terminal: bool) -> PpoTrainerSession {
    ...
    cfg.env.max_steps = 3;
    if terminal {
        cfg.env.max_angle_rad = 0.0;
    }
    let mut session = PpoTrainerSession::new_seeded_with_environments(cfg, 201, 3);
    let mut state = session.shared_state();
    for layer in [
        &mut state.policy.input,
        &mut state.policy.hidden,
        &mut state.policy.output,
        &mut state.value.input,
        &mut state.value.hidden,
        &mut state.value.output,
    ] {
...
fn references(session: &PpoTrainerSession) -> Vec<ReferenceStream> {
    std::iter::once(RolloutEnvironment {
        ...
        episode_return: session.episode_return,
        environment_rng: session.environment_rng.clone(),
        action_rng: session.action_rng.clone(),
    })
    .chain(session.additional_environments.iter().cloned())
    .map(|state| ReferenceStream { state, age: 0 })
    .collect()
}
...
fn pooled_gae_matches_forward_oracle_across_timeouts_and_live_cutoffs() {
    ...
    for _ in 0..4 {
        ...
        for stream in &mut streams {
            ...
            for _ in 0..2 {
                ...
                let result = stream
                    .state
                    .env
                    .step_with_rng(action.action, &mut stream.state.environment_rng);
                stream.age += 1;
                ...
                if ended {
                    ...
                    stream.state.current_observation = stream
                        .state
                        .env
                        .reset_with_rng(&mut stream.state.environment_rng);
                    stream.age = 0;
                }
            }
            ...
            for start in 0..rows.len() {
                ...
                for &(reward, terminal, ended) in &rows[start..] {
                    advantage +=
                        weight * (reward + (if terminal { 0.0 } else { 0.75 * 2.0 }) - 2.0);
                    if ended {
                        break;
                    }
                    weight *= 0.75 * 0.5;
        ...
        let std = (raw_advantages
            .iter()
            .map(|x| (x - mean).powi(2))
            .sum::<f64>()
            / raw_advantages.len() as f64)
            .sqrt()
            .max(1e-6);
        for i in 0..actual.returns.len() {
            assert!(
                (f64::from(actual.returns[i]) - expected_returns[i]).abs() < 2e-5,
                "stream-local return target"
            );
            let expected = (raw_advantages[i] - mean) / std;
            assert!(
                (f64::from(actual.advantages[i]) - expected).abs() < 2e-4,
                "one normalization over the pool"
            );
        }
        ...
        for (actual, expected) in session.additional_environments.iter().zip(&streams[1..]) {
            assert_eq!(
                stream_fingerprint(actual),
                stream_fingerprint(&expected.state)
            );
        }
        ...
        if let Some(last) = completed.last() {
            ...
            assert_eq!(
                ...
                completed.iter().copied().fold(f32::NEG_INFINITY, f32::max)
            );
        }
    }
